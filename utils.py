...
class MyDataset(Dataset):
    ...
    """
    def __init__(self,train_irt_params_path:Optional[str]=None, 
                 item_embeddings_path:Optional[str]=None, archor_count:int=5,
                 max_sample_count:int=1e10,random_seed:int=42):

        
        with open(train_irt_params_path,'r',encoding='utf-8') as f:
            lines = f.readlines()


        self.train_irt_params = [json.loads(line) for line in lines]
        self.item_embeddings = np.load(item_embeddings_path)
        self.max_sample_count = min(max_sample_count,len(self.train_irt_params))
    ...
    def __getitem__(self, idx):
        train_irt_param = self.train_irt_params[idx]
        irt_torch = torch.tensor([
            *(train_irt_param['ability']),
            *(train_irt_param['response']),
            train_irt_param['diff'],
            train_irt_param['disc'],
            ])

        embedding_torch = self.item_embeddings[train_irt_param['embedding_item_idx']]

        label_torch = torch.tensor([
            train_irt_param['target_diff'],
            train_irt_param['target_disc'],
            ])

        return irt_torch,embedding_torch,label_torch
...
if __name__=='__main__':
    ...
    irt_data_generator = IRTDataGenerator('./global_selected_400_train.jsonl','./full_fit/best_parameters.json','embedding_item2idx.json')
    py_irt_params = {

    }
    irt_data_generator.generate(100,5,'20260729')
